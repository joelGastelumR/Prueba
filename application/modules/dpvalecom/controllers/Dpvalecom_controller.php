          ...
          /* se revisa si es apto para baneo*/
           ...
           if($idCustomer == '' || $idCustomer == null || $idCustomer == 'null'){
            ...
            $data = $this->Dpvalecom_model->getSeguro( $this->getSession('idbranch'), $this->hash, $upFolio );
             ...
             if($sms['code'] == "A01"){
                   $response = ["status"=>'error',"message"=>"[A01] No se puede enviar el SMS, <b>El Canjeante no tiene registrado teléfono celular para enviar el SMS.</b><br> Favor de comunicarlo a Contact Center.<br>"];
                   die(json_encode($response));
             }
             if($sms['code'] == "A06"){
                   $response = ["status"=>'error',"message"=>"[A06] No se puede enviar el SMS, <b>El número telefónico del canjeante no es valido.</b><br> Favor de comunicarlo a Contact Center.<br>"];
                   die(json_encode($response));
             }
